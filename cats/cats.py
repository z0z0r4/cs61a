...
def accuracy(typed: str, source: str) -> float:
    ...
    if len(typed_words)==0 and len(source_words)==0:
        ...
        for t_word, s_word in zip(typed_words, source_words):
            if t_word == s_word:
                correct += 1
...
def autocorrect(typed_word: str, word_list: list[str], diff_function, limit: int) -> str:
    ...
    if typed_word in word_list:
        ...
        min_diff_correct_word=None
        min_diff = limit+1
        for word in word_list:
            diff=diff_function(typed_word, word, limit)
            if diff<=limit and diff < min_diff:
                min_diff=diff
                min_diff_correct_word=word
        if min_diff_correct_word==None:
            return typed_word
        else:
            return min_diff_correct_word
...
def furry_fixes(typed: str, source: str, limit: int) -> int:
    ...
    # BEGIN PROBLEM 6
    # 用递归写法会更好一些
    # length_difference = abs(len(typed) - len(source))
    # substitutions = 0

    # for t_char, s_char in zip(typed, source):
    #     if t_char != s_char:
    #         substitutions += 1


    # return substitutions + length_difference
    if limit<0:
        return 0
...
def minimum_mewtations(typed: str, source: str, limit: int) -> int:
    ...
    def helper(i, j, edits):
        ...
        if typed[i] == source[j]:
            ...
            add = helper(i, j + 1, edits + 1)
            remove = helper(i + 1, j, edits + 1)
            substitute = helper(i + 1, j + 1, edits + 1)
