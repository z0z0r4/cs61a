...
class Transaction:
    ...
    def report(self) -> str:
        ...
        if self.changed():
            "*** YOUR CODE HERE ***"
            changed_val = self.after - self.before
            if changed_val > 0:
                msg = "increased "
            else:
                msg = "decreased "
...
class BankAccount:
    ...
    def deposit(self, amount: int) -> int:
        ...
        if len(self.transactions) > 0:
            self.transactions.append(Transaction(self.transactions[-1].id + 1, before=self.balance, after=self.balance + amount))
        else:
            self.transactions.append(Transaction(0, before=self.balance, after=self.balance + amount))
    ...
    def withdraw(self, amount: int) -> int | str:
        ...
        if amount > self.balance:
            if len(self.transactions) > 0:
                self.transactions.append(Transaction(self.transactions[-1].id + 1, before=self.balance, after=self.balance))
            else:
                self.transactions.append(Transaction(0, before=self.balance, after=self.balance))
...
class VirFib():
    ...
    def next(self):
        "*** YOUR CODE HERE ***"
        instance = VirFib()
        instance.last_value = self.value
        instance.value = self.value + self.last_value
        return instance
