    ...
    def nearest_bee(self) -> Bee | None:
        ...
        if len(self.place.bees) == 0:
            ...
            while current_place is not None:
                if current_place.is_hive or times > self.upper_bound: # 距离超过仍未找到则返回 None
                    break
                if times < self.lower_bound:
                    current_place = current_place.entrance
                    times+=1
                    continue # 距离不足则跳过 place
...
class ShortThrower(ThrowerAnt):
    ...
    def __init__(self, health: int = 1):
        """Create a ShortThrower with a HEALTH quantity."""
        super().__init__(health)
...
class LongThrower(ThrowerAnt):
    ...
    def __init__(self, health: int = 1):
        """Create a LongThrower with a HEALTH quantity."""
        super().__init__(health)
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount: float):
        ...
        for bee in list(current_place.bees):
            bee.reduce_health(amount + self.damage if self.health <= 0 else amount)
        # END Problem 5
...
class TankAnt(ContainerAnt):
    ...
    def action(self,  gamestate: GameState):
        ...
        super().action(gamestate=gamestate)
