...
def balanced(m):
    ...
    left_mass = total_mass(left_thing) if is_mobile(left_thing) else mass(left_thing)
    right_mass = total_mass(right_thing) if is_mobile(right_thing) else mass(right_thing)

    if left_mass * length(left(m)) == right_mass * length(right(m)):
        if balanced(left_thing) and balanced(right_thing):
            return True
