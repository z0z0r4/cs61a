...
def do_define_form(expressions, env: Frame):
    ...
    if scheme_symbolp(signature):
        ...
        lambda_procedure = LambdaProcedure(formals=formals, body=body, env=env)
...
def do_lambda_form(expressions, env):
    ...
    "*** YOUR CODE HERE ***"
    body = expressions.rest
    return LambdaProcedure(formals=formals, body=body, env=env)
    # END PROBLEM 7
...
def do_and_form(expressions, env):
    ...
    while curr is not nil:
        ...
        return last_result if last_result is not None else True
    # END PROBLEM 12
...
def make_let_frame(bindings, env):
    ...
    while curr_bindings is not nil:
        curr_binding = curr_bindings.first
        validate_form(curr_binding, 2, 2)

        name = curr_binding.first
        if not scheme_symbolp(name):
            raise SchemeError("non-symbol name in let binding")
        if name in symbols:
            raise SchemeError("duplicate name")

        val = curr_binding.rest.first
        names = Link(name, names)
        vals = Link(scheme_eval(val, env), vals)
        curr_bindings = curr_bindings.rest
        symbols.add(name)
